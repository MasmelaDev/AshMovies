...
main.addEventListener("click", showMovie);
...
function navigator() {
  if (location.hash.startsWith("#Trending")) {
    changeSection("Trending", "/trending/movie/day", {
      params: { region: "US", page: 3 },
    });
    ...
    changeSection("Popular", "/movie/popular", {
      params: { region: "US", page: 3 },
    });
...
function showGenre() {
  const genres__resultContainer = document.querySelector(
    ".genres__resultContainer"
  );
  ...
  genres__resultContainer.append(
    genres__resultContainerTitle,
    genres__resultMoviesContainer
  );

  getAndAppendMovies(".genres__resultMoviesContainer", "/discover/movie", {
    params: { with_genres: genre[1] },
  });

  genres__resultContainer.classList.add("active");
  genres__resultMoviesContainer.classList.add("active");
  footerObserverInfiniteScroll.observe(footer);
...
async function changeSection(section, path, optionalConfig = {}) {
  ...
  const changeSection__li = [
    ...document.querySelectorAll(".changeSection__li"),
  ];
  ...
  await getAndAppendMovies(".moviesContainer", path, optionalConfig);
...
function showSearch() {
  ...
  search.append(search__resultContainerTitle, search__resultMoviesContainer);

  getAndAppendMovies(".search__resultMoviesContainer", "/search/movie", {
...
async function showMovie(entry) {
  ...
  if (entry.target.className == "movieImg") {
    ...
    const movieDescription__imgContainer = document.querySelector(
      ".movieDescription__imgContainer"
    );
    ...
    const movieDescription__title = document.querySelector(
      ".movieDescription__title"
    );
    movieDescription__title.innerText = data.title;
    const movieDescription__overview = document.querySelector(
      ".movieDescription__overview"
    );
...
const movieDescription__back = document.querySelector(
  ".movieDescription__back"
);
